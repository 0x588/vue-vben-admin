...
import { checkCaptcha, getCaptcha } from '@/api/sys/login'
...
async function getPictrue() {
  ...
  if (res.data.code === 200) {
    ...
    pointImage.value = res.data.data.jigsawImageBase64
    text.value = `${t('component.captcha.point')}`
  }
  else {
...
<template>
  <div style="position: relative">
    ...
    >
      <span class="verify-msg">{{ text }}<img :src="`data:image/png;base64,${pointImage}`"></span>
    </div>
  </div>
</template>
